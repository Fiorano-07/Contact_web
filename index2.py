...
def Database(search=None):
    ...
    if search:
        cursor.execute("SELECT * FROM `member` WHERE `lastname` LIKE ? ORDER BY `lastname` ASC", ('%'+search+'%',))
    else:
...
def SearchRecord(lastname):
    tree.delete(*tree.get_children())
    conn = sqlite3.connect("pythontut.db")
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM `member` WHERE `lastname` LIKE ?", ('%'+lastname+'%',))
    fetch = cursor.fetchall()
    for data in fetch:
        tree.insert('', 'end', values=(data))
    cursor.close()
...
SearchBar = Frame(Mid, width=300, bg="#6666ff")
SearchBar.pack(side=LEFT, pady=10)
...
lbl_search = Label(SearchBar, text="Search Lastname:", font=('arial', 12), bg="#6666ff")
lbl_search.pack(side=LEFT, padx=10)
search_entry = Entry(SearchBar, font=('arial', 12), width=15)
search_entry.pack(side=LEFT)
btn_search = Button(SearchBar, text="Search", command=lambda: SearchRecord(search_entry.get()))
btn_search.pack(side=LEFT, padx=10)
btn_clear = Button(SearchBar, text="Clear", command=ClearSearch)
